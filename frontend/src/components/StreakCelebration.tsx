  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <p className="font-sans font-extrabold text-2xl sm:text-3xl text-cream-800 dark:text-cream-100">
          ¡{streak} días seguidos!
        </p>
        <p className="font-handwritten text-cream-500 dark:text-cream-400 text-lg mt-1">
          {level.label} — {habitName}
        </p>
        <p className="font-sans text-cream-700 dark:text-cream-300 text-base mt-4 leading-relaxed">
          {level.message}
        </p>
        <button
          onClick={onDismiss}
          className="mt-6 px-6 h-11 rounded-full bg-terracotta-600 text-cream-50 text-sm font-bold hover:bg-terracotta-700 transition-colors focus:outline-none focus:ring-2 focus:ring-terracotta-400 focus:ring-offset-1"
        >
          ¡Genial!
        </button>
      </div>
