...
export default function HabitRow({ habit, days, monthStr, today, totalDays, completions, showStreak = true, onToggle }: HabitRowProps) {
  ...
  const [celebrating, setCelebrating] = useState<StreakLevel | null>(null)
  ...
  useEffect(() => {
    if (!showStreak || !streakLevel) return
    ...
    if (streakLevel.days > lastCelebrated) {
      ...
      setCelebrating(streakLevel)
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [habit.id, streakLevel?.days, showStreak])
  ...
  return (
    <>
      <tr className="border-b border-cream-200 dark:border-cream-600 group hover:bg-cream-100/50 dark:hover:bg-cream-700/50 transition-colors">
        {/* Habit info cell */}
        <td className="py-1.5 pr-2 sticky left-0 bg-cream-50 dark:bg-cream-800 z-10 min-w-[150px] max-w-[190px]">
          <div className="flex items-center gap-1.5">
            ...
            <div className="overflow-hidden flex-1 min-w-0">
              <div className="font-sans font-bold text-xs text-cream-800 dark:text-cream-100 leading-tight truncate flex items-center gap-1">
                {habit.icon} {habit.name}
                {streakLevel && (
                  <span
                    className="flex items-center gap-0.5 text-[10px] font-bold shrink-0"
                    style={{ color: streakLevel.color }}
                    title={`${streak}-day streak — ${streakLevel.label}`}
                    aria-label={`${streak}-day streak, ${streakLevel.label}`}
                  >
            ...
            <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
              ...
              <button
                ...
                title="Archive habit (hide without deleting)"
