...
export default function HabitGrid({ habits, year, month, completions, isError, onToggle }: HabitGridProps) {
  ...
  if (isError) {
    return (
      <div className="flex items-center justify-center gap-2 py-10 text-red-500 font-sans text-sm">
        ...
        Failed to load habits. Check your connection and try refreshing.
      </div>
    )
  }
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-cream-300 dark:border-cream-600">
      <table className="text-xs w-full" style={{ borderCollapse: 'separate', borderSpacing: '2px 2px' }}>
        <thead>
          <tr className="bg-cream-100 dark:bg-cream-700">
            <th className="py-2 px-2 text-left font-bold text-cream-700 dark:text-cream-200 sticky left-0 bg-cream-100 dark:bg-cream-700 z-10 min-w-[150px] rounded-sm">
              HABIT
            </th>
            {days.map((d) => {
              ...
              return (
                <th
                  key={d}
                  className={[
                    'text-center font-bold py-2 rounded-sm',
